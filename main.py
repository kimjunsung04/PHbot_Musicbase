...
def print_progress(iteration, total, prefix='Progress:', suffix='Complete', decimals=1, bar_length=10):
    ...
    bar = ":yellow_square:" * filled_length + ':black_large_square:' * (bar_length - filled_length)
    return bar
...
async def on_message(message):
    ...
    if message.content.startswith(f"{prf}재생"):
        try:
            ...
            if url == "":
                return await message.channel.send("+재생 `유튜브 링크` - 입력한 유튜브 링크를 재생해줍니다.\n+재생 `노래제목` - 입력한 노래제목을 유튜브에서 찾아줍니다.")
            try:
                if message.guild.voice_client.channel.id != message.author.voice.channel.id:
                    if server.id in nowplay:
                        return await message.channel.send(f"이미 **{message.guild.voice_client.channel}** 에서 재생중입니다.")
                    else:
            ...
            if "http" in url:
                if "playlist?list=" in url:
                    return await message.channel.send(embed=discord.Embed(title=f"재생▶",
            ...
            if is_p == True:#예약
                ...
                if live == True: cho = f"[**{mdp[3]}**]"
                else: cho = f"[**{mdp[3]}초**]"
                mant = f"[{title}]({m_urld}) {cho} 를\n**{dgy}번** 대기열에 예약하였습니다!"
                return await message.channel.send(embed=discord.Embed(title=f"예약📥",
                                                                      description=f"{mant}",
                                                                      color=0x0170ed))
            server = message.guild
            if live == True:
            ...
            if live == True: cho = f"[**{now_p[3]}**]"
            else: cho = f"[**{now_p[3]}초**]"
            ...
            if mis == False:
                ...
                voice.play(source, after=lambda e: check_queue(message.guild.id, message.author.id))
            else:
                message.guild.voice_client.play(source, after=lambda e: check_queue(message.guild.id, message.author.id))

            await message.channel.send(embed=discord.Embed(title=f"재생▶",
                                                                  description=f"{mant}",
                                                                  color=0x0170ed))
    ...
    if message.content.startswith(f"{prf}검색"):
        ...
        print(url)
        if "http" in url:
            return await message.channel.send(embed=discord.Embed(title=f"검색🔍",
                                                                  description=f"링크는 검색할수 없습니다!",
                                                                  color=0x0170ed))
        server = message.guild
        ...
        while(i <= 4):
            ...
            try:
                ...
                break
            i+=1
        tg = await message.channel.send(embed=discord.Embed(title=f"{url_mdk}에 대한 검색결과🔍",
                                                                  description=f"{con}",
                                                                  color=0x0170ed))
        try:
            ...
            return await message.channel.send(f"PH봇이 반응을 추가할수 없습니다!\n`반응 추가하기` 권한을 확인해주세요!\n<@{message.author.id}>")
        try:
            def diary_write_check(reaction, user):
                return user == author and str(reaction) in ["1️⃣", "2️⃣", "3️⃣", "4️⃣", "5️⃣", "❌"] and tg.id == reaction.message.id # 이모지 리액션 부분
            reaction, user = await client.wait_for('reaction_add',timeout=30.0,check=diary_write_check)# 이모지 리액션 부분
        ...
        if str(reaction.emoji) == "❌":
            await tg.clear_reactions()
            return await tg.edit(embed=discord.Embed(title=f"검색🔍",
                                                                  description=f"검색을 취소하였습니다!",
                                                                  color=0x0170ed))
        ...
        if is_p == True:#예약
            ...
            if server.id in musiclist:
                ...
                musiclist[server.id] = [mdp] #딕셔너리 쌍 추가
            return await tg.edit(embed=discord.Embed(title=f"예약📥",
                                                                  description=f"{mant}",
                                                                  color=0x0170ed))
        server = message.guild
        start = time.time()
        ...
        if mis == False:
            ...
            message.guild.voice_client.play(source, after=lambda e: check_queue(message.guild.id, message.author.id))
        await tg.edit(embed=discord.Embed(title=f"검색🔍",
                                                              description=f"{mant}",
                                                              color=0x0170ed))
            ...
            if message.guild.voice_client.channel.id == message.author.voice.channel.id:
                ...
                if is_p == True:
                    if int(qd[2]) == message.author.id:
                        ...
                        return await message.channel.send(embed=discord.Embed(title=f"스킵⏭",
    ...
    if message.content == f"{prf}지금노래":
        try:
            ...
            if str(now_p[3]) == "LIVE":
                return await message.channel.send(embed=discord.Embed(title=f"지금노래🎵",
                                                                          description=f"[{now_p[1]}]({now_p[0]}) [**{now_p[3]}**] 가 재생중입니다.\n재생한 사람 : <@{now_p[2]}>",
                                                                          color=0x0170ed))
            await message.channel.send(embed=discord.Embed(title=f"지금노래🎵",
                                                                  description=f"[{now_p[1]}]({now_p[0]}) 가 재생중입니다.\n{print_progress(int(time.time()) - int(now_p[4]),now_p[3])}({int(time.time()) - int(now_p[4])}/{now_p[3]})초\n재생한 사람 : <@{now_p[2]}>",
                                                                  color=0x0170ed))
        except IndexError:
            return await message.channel.send("현재는 재생중인 노래가 없습니다.")
        ...
        try:
            if message.guild.voice_client.channel.id == message.author.voice.channel.id:
                try:
                    ...
                    return await message.channel.send(embed=discord.Embed(title=f"대기열삭제🗑",
                                                                          description=f"노래가 재생중이 아닙니다.",
                                                                          color=0x0170ed))
                except KeyError:
                    return await message.channel.send(embed=discord.Embed(title=f"대기열삭제🗑",
                ...
                if int(qd[2]) == message.author.id:
                    ...
                    await message.channel.send(embed=discord.Embed(title=f"대기열삭제🗑",
        ...
        try:
            ...
            try:
                while (True):
                    ...
                    if str(da[3]) == "LIVE": da_cho = f"[**{da[3]}**]"
                    else: da_cho = f"[**{da[3]}초**]"
            ...
            if page_d > 10:
                ...
                page = 1
                embed=discord.Embed(title='대기열🗃', description=f"{con}", color=0x0170ed, timestamp=message.created_at)
                embed.set_footer(text=f"페이지 : {page} / {max_p}", icon_url=client.user.avatar_url)
                ...
                while(True):                  
                    ...
                    await tg.add_reaction("▶")
                    def diary_write_check(reaction, user):
                        return user == author and str(reaction) in ["◀","⏹","▶"] and tg.id == reaction.message.id # 이모지 리액션 부분
                    reaction, user = await client.wait_for('reaction_add',timeout=30.0,check=diary_write_check)# 이모지 리액션 부분
                    if str(reaction.emoji) == "◀":
                        await tg.clear_reactions()
                        if page-1 == 0:
                ...
                if con == "":
                    return await message.channel.send(embed=discord.Embed(title=f"대기열🗃",
                                                                          description=f"현재 대기열이 비어있습니다.",
                                                                          color=0x0170ed))
                await message.channel.send(embed=discord.Embed(title=f"대기열🗃",
                                                                      description=f"{con}",
                                                                      color=0x0170ed))
        except KeyError:
